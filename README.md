...
| 명령어 | 권한 | 설명 |
|--------|------|------|
| `/mayhem start` | 게임 모드 선택 GUI 열기 | `asurajang.admin` |
| `/mayhem stop` | 게임 강제 종료 | `asurajang.admin` |
| `/mayhem reload` | 설정 리로드 | `asurajang.admin` |
| `/mayhem list` | 전체 증강 목록 보기 | `` |
| `/mayhem list prism` | 프리즘 증강 목록 보기 | `` |
| `/mayhem status` | 내 증강 확인 | `` |
...
- 같은 팀끼리는 서로 피해를 주지 않습니다.
- 아래 **기지 모드** 또는 **야생 모드**를 추가로 선택할 수 있음

### ⚔️ 개인전
- 모두가 적, 전장 곳곳에 흩어져 시작

### 🚩 기지 모드 (옵션, 팀전 전용)
- 양 팀 진영에 거점이 생기고, 거점을 지키는 가디언을 쓰러뜨리면 거점이 파괴됩니다.
- 가디언은 여러 번의 목숨을 가지고 있어 한 번 쓰러뜨려도 시간이 지나면 더 강해져서 부활합니다.
- 죽으면 팀 거점에서 부활합니다.

**🚀 거점 공격 (옵션)**
- 가디언이 범위 안에 들어온 상대팀을 유도형 투사체로 직접 공격하며 라이프가 깍일수록 공격 속도가 증가합니다.

### 🌳 야생 모드 (옵션)
- 시간이 지날수록 월드 보더가 점점 좁아져 전투 범위가 강제로 줄어듭니다.
...
| 능력치 | 효과 |
|--------|------|
| 🛡️ 맷집 | 최대 체력 +1칸, 공격력 +0.2, 이동 속도 +0.005 |
| 💪 근력 | 최대 체력 +0.25칸, 공격력 +0.5, 이동 속도 +0.005 |
| 🏃 민첩 | 최대 체력 +0.25칸, 공격력 +0.2, 이동 속도 +0.01 |
...
<tbody>
<tr><td align="center"><img src="docs/icons/trim_spire.png" width="64"></td><td>맹추<br><sub>(추격자+사냥 본능)</sub></td><td>같은 적 연속 공격 시 피해가 3%씩 추가 증가합니다 (최대 30스택). 처치 시 스택이 50% 감소합니다.</td></tr>
